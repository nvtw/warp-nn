...
def test_forward(capsys, device, dtype, ndim, negative_slope):
    if not is_device_available(device):
        pytest.skip(f"Device '{device}' is not available")
    check_forward(
        warp_activation=nn.LeakyReLU(negative_slope=negative_slope),
        torch_activation=torch.nn.LeakyReLU(negative_slope=negative_slope),
        device=device,
        dtype=dtype,
        ndim=ndim,
    )
...
@pytest.mark.parametrize("device", ["cpu", "cuda"])
def test_gradients(capsys, device, dtype, ndim, negative_slope):
    if not is_device_available(device):
        pytest.skip(f"Device '{device}' is not available")
    check_gradients(
        warp_activation=nn.LeakyReLU(negative_slope=negative_slope),
