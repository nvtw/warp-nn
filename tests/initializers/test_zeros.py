...
def test_initialization(capsys, device, dtype, requires_grad):
    ...
    # in-place: False
    array = wp.ones(shape=(10,), dtype=dtype, device=device, requires_grad=requires_grad)
    output = initializers.zeros(array, inplace=False)
    assert output is not array
    assert np.allclose(array.numpy(), 1.0)
    assert np.allclose(output.numpy(), 0.0)
    _check(output)
    # in-place: True
    array = wp.ones(shape=(10,), dtype=dtype, device=device, requires_grad=requires_grad)
    output = initializers.zeros(array, inplace=True)
