...
def test_safetensor_archive_loads_and_dequantizes_e4m3(tmp_path):
    ...
    write_safetensors(tmp_path / "model.safetensors", {"weight": ("F8_E4M3", bits.shape, bits.tobytes())})
    archive = SafeTensorArchive(tmp_path)
    ...
    wp.launch(_dequantize_e4m3_kernel, dim=bits.size, inputs=[packed, scale, output], device="cuda:0")
