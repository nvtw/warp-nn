...
def _create_kernels(config: KernelConfig, *, negative_slope: float):
    ...
    def kernel_1d(input: wp.array1d[Any], output: wp.array1d[Any]):
        wp.static(negative_slope)  # hack to force kernel specialization
        i = wp.tid()
...
class LeakyReLU(Module):
    def __init__(self, *, negative_slope: float = 0.01) -> None:
        r"""Leaky Rectified Linear Unit (Leaky ReLU) activation function.
        ...
        :param negative_slope: The negative slope value for the Leaky ReLU function.
        """
        super().__init__()
