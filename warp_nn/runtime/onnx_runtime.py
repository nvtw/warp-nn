...
These kernels are intentionally simpler than the tiled kernels used by
:class:`~warp_nn.modules.layers.Linear`, :class:`~warp_nn.modules.activations.ELU`,
and :class:`~warp_nn.modules.layers.LSTMCell`.  Policy networks are tiny
(batch=1, hidden<=128), so the lightweight path keeps compile pressure low
and remains CUDA-graph capturable after warmup.
