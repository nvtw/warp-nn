...
(() => {
  ...
  function frame(v,maximum=1.15){if(!v.length)return;let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity;for(const n of v){minX=Math.min(minX,n.x-110);maxX=Math.max(maxX,n.x+110);minY=Math.min(minY,n.y-90);maxY=Math.max(maxY,n.y+90)}scale=Math.min(maximum,(width-80)/(maxX-minX),(height-80)/(maxY-minY));ox=width/2-(minX+maxX)/2*scale;oy=height/2-(minY+maxY)/2*scale;draw()}
  ...
  function wrapWords(text,maxWidth){const words=text.split(/\s+/),lines=[];let line='';for(const word of words){const next=line?line+' '+word:word;if(line&&ctx.measureText(next).width>maxWidth){lines.push(line);line=word}else line=next}if(line)lines.push(line);return lines}
  ...
  function drawNode(n){const p=screen(n), hit=n===selected||n===hovered, match=query&&((n.fullName||n.label)+' '+(n.subtitle||'')).toLowerCase().includes(query);if(n.type==='tensor'){ctx.beginPath();ctx.arc(p.x,p.y,Math.max(2.5,n.r*scale),0,Math.PI*2);ctx.fillStyle=match?'#e84f76':palette.tensor;ctx.fill();return}
    const factor=Math.max(.62,Math.min(1,scale)),w=Math.max(130,Math.min(220,104+n.label.length*4.4))*factor,title=n.layer==null?n.label:n.label.replace(/^Layer \d+ · /,''),hasSubtitle=scale>.48;
    ctx.font=`650 ${Math.max(9,13*Math.min(1,scale))}px ui-sans-serif,system-ui`;const lines=wrapWords(title,w-42),contentHeight=(n.layer==null?0:12)+lines.length*15+(hasSubtitle?17:0),h=Math.max(58*factor,contentHeight+18);
    ...
    if(scale>.22){const x=p.x-w/2+24;let y=p.y-contentHeight/2;ctx.textAlign='left';ctx.textBaseline='middle';if(n.layer!=null){ctx.fillStyle='#7b8798';ctx.font='700 8.5px ui-sans-serif,system-ui';ctx.fillText(`LAYER ${n.layer}`,x,y+5);y+=12}ctx.fillStyle='#233044';ctx.font=`650 ${Math.max(9,13*Math.min(1,scale))}px ui-sans-serif,system-ui`;for(const line of lines){ctx.fillText(line,x,y+7);y+=15}if(hasSubtitle){ctx.fillStyle='#717d8f';ctx.font='10.5px ui-sans-serif,system-ui';ctx.fillText(n.subtitle,x,y+8)}}
  }
