...
#play{flex:0 0 38px;width:38px;border:0;color:white;background:linear-gradient(135deg,#7065f0,#5147d8);box-shadow:0 7px 18px rgba(81,71,216,.28);font-size:15px}.icon{display:inline-block;width:12px;text-align:center}
...
<section class="transport" aria-label="Playback controls">
  <button id="play" title="Play or pause"><span class="icon">Ⅱ</span></button>
...
const jointGeo=new THREE.SphereGeometry(.055,18,12),boneGeo=new THREE.CylinderGeometry(.027,.027,1,12,1,false);boneGeo.translate(0,.5,0);
const jointMat=new THREE.MeshPhysicalMaterial({color:0x6258eb,roughness:.27,metalness:.04,clearcoat:.55,clearcoatRoughness:.3});const boneMat=new THREE.MeshPhysicalMaterial({color:0x6258eb,roughness:.32,metalness:.03,clearcoat:.35});
...
const play=document.querySelector('#play');function setPlaying(value){playing=value;play.querySelector('.icon').textContent=playing?'Ⅱ':'▶'}play.onclick=()=>setPlaying(!playing);document.querySelector('#speed').onchange=e=>speed=Number(e.target.value);
...
resetCamera();pose();loading.style.opacity=0;setTimeout(()=>loading.remove(),320);
