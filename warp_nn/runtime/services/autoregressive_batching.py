...
"""Model-neutral continuous-batching adapter for native autoregressive runners."""
...
class AutoregressiveBatchPayload:
    token_ids: tuple[int, ...]
    temperature: float
    top_k: int
    top_p: float
...
@dataclass
class _Slot:
    payload: AutoregressiveBatchPayload
    logits: Any = None
...
class AutoregressiveBatchExecutor:
    """Sample tokens around a shared-weight native batch decoder."""
    ...
    def admit(self, slot: int, request: BatchRequest, reuse_prefix: bool) -> int:
        ...
        if reuse_prefix:
            ...
            if (
                ...
                raise RuntimeError("Batch prefix state no longer matches")
    ...
    def select_decode_bucket(self, active_count: int) -> int:
        if not 0 < active_count <= self.max_batch_size:
            raise ValueError("invalid decode batch size")
    ...
    def decode(self, slots: list[int], bucket_size: int):
        if bucket_size not in (1, 2, 4, 8) or bucket_size > self.max_batch_size:
            raise ValueError("Decoder bucket is unsupported")
        if bucket_size == 1 and len(slots) != 1:
            raise ValueError("Single-slot bucket requires one active request")
    ...
    @staticmethod
    def _payload(request: BatchRequest) -> AutoregressiveBatchPayload:
        if not isinstance(request.payload, AutoregressiveBatchPayload):
            raise TypeError("Batch request has an invalid payload")
        return request.payload

    def _state(self, slot: int) -> _Slot:
        state = self._slots[slot]
        if state is None:
            raise RuntimeError("Batch slot is not admitted")
        return state
